...
void solve(){
    int n,q;
    cin>>n>>q;
    vector<node> a(n,node(0));
    tr(e,a){
        int x;cin>>x,e=x;
    }
    SegTree<node> seg(a,[](node a,node b)->node{
        return node(a.s+b.s,max(a.p,a.s+b.p));
    });
    while(q--){
        int t,x,y;
        cin>>t>>x>>y;
        if(t==1){
            seg.update(0,n-1,x-1,y);
        }
        else{
            cout<<max(0ll,seg.get(0,n-1,x-1,y-1).p)<<"\n";
        }
    }
}
