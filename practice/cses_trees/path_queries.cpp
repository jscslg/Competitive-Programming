...
void solve(){
    ...
    dfs(0);
    SegTree<int> seg(vi(n,0),[](int x,int y){
        return x+y;
    });
    fr(i,0,n){
        seg.update(0,n-1,in[i],a[i]);
        if(en[i]!=n-1) seg.update(0,n-1,en[i]+1,-a[i]);
    }
    while(q--){
        int x,y,z;
        cin>>x>>y,y--;
        if(x==1){
            cin>>z;
            seg.update(0,n-1,in[y],z-a[y]);
            if(en[y]!=n-1) seg.update(0,n-1,en[y]+1,-(z-a[y]));
            a[y]=z;
        }
        else{
            cout<<seg.get(0,n-1,0,in[y])<<"\n";
        }
    }
}
